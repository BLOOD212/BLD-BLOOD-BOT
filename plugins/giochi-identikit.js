...
let handler = async (m, { conn, usedPrefix, command, text }) => {
    ...
    const animali = [
        ...
        { nome: 'canguro', difficolta: '🟡 MEDIO', indizi: ['Le femmine ont una tasca sul ventre', 'Usa la grande coda per non perdere l\'equilibrio', 'Non è capace di camminare all\'indietro', 'È un marsupiale che si sposta saltando in Australia'] },
        ...
        { nome: 'ippopotamo', difficolta: '🟡 MEDIO', indizi: ['Il suo sudore rosso funge da crema solare', 'Passa la giornata in acqua per non brucare la pelle', 'Anche se cicciottello, corre più veloce di un uomo', 'È tra gli animali più territoriali e pericolosi d\'Africa'] },
        ...
        { nome: 'kiwi', difficolta: '🔴 IMPOSSIBILE', indizi: ['Depone l\'uovo più grande del mondo in proporzione al suo corpo', 'Le suas piume somigliano moltissimo a dei peli ruvidi', 'Ha le narici poste proprio sulla punta del suo lungo becco', 'È un uccello notturno della Nuova Zelanda che non vola'] },
        ...
        { nome: 'aye aye', difficolta: '🔴 IMPOSSIBILE', indizi: ['Ha un dente medio lunghissimo e scheletrico', 'Usa l\'ecolocalizzazione picchiettando sui tronchi d\'albero', 'È un lemure notturno avvolto da superstizioni locali', 'Vive solo in Madagascar ed ha grandi occhi arancioni'] },
    ...
    if (command === 'identikit') {
        if (currentGame[chatId]) return m.reply(`⚠️ C'è già una sessione investigativa attiva! Rispondi con \`${usedPrefix}indovina [nome]\``);
        ...
        currentGame[chatId] = {
            ...
            indice: 1,
            timer: setInterval(() => {
                if (currentGame[chatId] && currentGame[chatId].indice < animale.indizi.length) {
                    let corrente = currentGame[chatId].indice;
                    let indizioCorrente = currentGame[chatId].indizi[corrente];
                    ...
                    fallimento += `🤷‍♂️ Ritenta l\'indagine avviando un nuovo caso con \`${usedPrefix}identikit\`!`;
                    
                    conn.sendMessage(chatId, { text: fallimento });
                    clearInterval(currentGame[chatId].timer);
    ...
    if (command === 'indovina') {
        if (!currentGame[chatId]) return m.reply(`❌ Nessun caso aperto al momento. Digita \`${usedPrefix}identikit\` per iniziare.`);
        if (!text) return m.reply(`⚠️ Specifica il nome di un animale! Esempio: \`${usedPrefix}indovina elefante\``);
        ...
        if (tentativo === dati.animale) {
            ...
            delete currentGame[chatId] return conn.sendMessage(chatId, { text: ris, mentions: [vincitore] }, { quoted: m });
        } else {
...
handler.tags = ['game'];
handler.command = /^(identikit|indovina)$/i;
handler.group = true;
