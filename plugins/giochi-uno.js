...
// FUNZIONE PER INVIO MESSAGGIO INTERATTIVO COMPATIBILE iOS
async function sendUnoMessage(conn, chat, s, caption, quoted) {
    const imgBuffer = await generaGrafica(s);
    const upload = await conn.prepareWAMessageMedia({ image: imgBuffer }, { upload: conn.waUploadToServer });
    ...
    const message = {
        interactiveMessage: {
            ...
            body: { text: caption },
            footer: { text: "Usa i bottoni o scrivi il numero della carta" },
            nativeFlowMessage: {
                buttons: [
                    {
                        name: "quick_reply",
    ...
    return conn.relayMessage(chat, { viewOnceMessage: { message } }, { messageId: quoted.key.id });
}
...
let handler = async (m, { conn }) => {
    ...
    await sendUnoMessage(conn, chat, unoSession[chat], `🃏 *UNO MATCH*\n🎨 Colore: *${unoSession[chat].currentColor}*`, m);
}
...
handler.before = async (m, { conn }) => {
    ...
    // Estrazione comando da bottone per iOS/Android
    let msgText = (m.text || m.body || '').trim().toLowerCase()
    const interactiveResp = m.message?.interactiveResponseMessage || m.message?.viewOnceMessage?.message?.interactiveResponseMessage;
    
    if (interactiveResp?.nativeFlowResponseMessage?.paramsJson) {
        msgText = JSON.parse(interactiveResp.nativeFlowResponseMessage.paramsJson).id;
    }
    ...
    await sendUnoMessage(conn, chat, s, `${report}\n🎨 Colore: *${s.currentColor}*`, m);
}
